...
A price drop has been detected for an item you are tracking!
...
def get_product_details(url_to_check):
    ...
    try:
        ...
        print(f"A problem happened while connecting to URL: {e}")
        return "N/A", 0.0
...
def check_prices():
    print("Hourly price check has started.")
    ...
    for product in products:
        ...
        if 0 < new_price < product.price:
            print(f"PRICE DROP DETECTED! Product: {product.title[:30]}")
            ...
            db.session.commit()
            print("Database updated with the new price.")
        else:
...
def run_scheduler():
    print("Background scheduler started. It will check prices every hour.")
...
def track_product():
    ...
    if not data or 'product_url' not in data or 'user_email' not in data:
        return jsonify({'message': 'Incomplete information sent.'}), 400
    ...
    if not product:
        print(f"New product: {product_url}. Fetching details...")
        title, price = get_product_details(product_url)

        if title == "N/A" or price == 0.0:
            return jsonify(
                {'message': 'Could not retrieve product information from Amazon. Please check the URL.'}), 400
    ...
    return jsonify({'message': f'Success! "{product.title[:30]}..." is now being tracked.'}), 201
...
You can click the link below to buy now:
{product.url}
...
def run_scheduler():
    print("Background scheduler started, will check prices every hour.")
    schedule.every().hour.do(job)
    while True:
        schedule.run_pending()
        time.sleep(1)
...
def track_product():
    ...
    if not product:
        print(f"New product detected: {product_url}. Scraping details...")
        title, price = get_product_details(product_url)
...
if __name__ == "__main__":
    print("--- E-POSTA TESTI MANUEL OLARAK BASLATILDI ---")
    with app.app_context():
        job()
    print("--- E-POSTA TESTI TAMAMLANDI ---")
